...
wkd=$syn_report_path/rfiles/REPORTDELIVERY/GMO/$weekofyear
#emdf=$wkd/$emd".txt"
emdf=$wkd/$cday".txt"

#cldate=$(date +"%Y%m%d%H%M")
scemaillogpath=$dls_path
scemaillog=$scemaillogpath/"GMO"$cdate".log"
touch $scemaillog
...
##	python /syndata/syn-reports/gmosendscheduledemail.py $sct
##else
...
if [ -f "${rsf}" ]; then
        ...
        echo "Sending email for MSGT${sct}" >> $scemaillog
        echo "Sent email for MSGT${sct}" >> $scemaillog
else
