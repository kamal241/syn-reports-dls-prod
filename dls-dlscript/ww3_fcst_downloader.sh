...
downloaddatapath=$dls_path"/dls-data/WW3/"$cdate/$1
echo $downloaddatapath >> $dls_path/dls-ww3-prod.log
mkdir -p $downloaddatapath 
echo $urldirpath
if [ -d $urldirpath ]; then
	ddataurlsf="$cdate"_"$1"_"ww3_fcst_urls.csv"
	didxurlsf="$cdate"_"$1"_"ww3_fcst_idx_urls.csv"
